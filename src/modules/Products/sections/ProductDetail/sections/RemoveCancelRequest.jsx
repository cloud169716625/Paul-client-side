...
import { removeCancelRequest } from 'store/Actions/products';

export const RemoveCancelRequest = ({ show, setShow, id }) => {
    const { product, loading } = useSelector((state) => state?.products);
    const dispatch = useDispatch();

    return (
        <Modal
            heading="Remove Cancellation Request"
            ...
            customBody={
                <div className="mb-[32px] text-center">
                    ...
                    <div className='p-[20px] pt-[5px] pb-[10px] text-[#474761] w-[300px] ml-[auto] mr-[auto]'>
                        Are you sure you wish to remove this cancellation request?
                    </div>
                </div>
            }
            initialValues={{ id }}
            submitText="Remove Cancellation Request"
            loading={loading}
            handleSubmit={async () => {
                await dispatch(removeCancelRequest(product?.id));
                setShow(false);
            }}
            show={show}
            setShow={setShow}
